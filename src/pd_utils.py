...
def generate_choice_set_df():
    ...
    tc1_list = compute_tc_pd_current_mode(modo_PR, nivels_PR, nivels_api)
    tv1_list = compute_tv_pd_current_mode(modo_PR, nivels_PR, nivels_api)
    te1_list = compute_te_pd_current_mode(modo_PR, nivels_PR)
    
    label2_list = [modo_nuevo] * 8
    c2_list = compute_c_pd_new_mode()
    tc2_list = compute_tc_pd_new_mode()
    tv2_list = compute_tv_pd_new_mode(modo_PR, tv1_list[0], tc1_list[0], min(tc2_list), nivels_api)
    te2_list = compute_te_pd_new_mode(te1_list[0])
